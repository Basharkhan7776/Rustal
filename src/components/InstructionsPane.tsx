...
export const InstructionsPane: React.FC<InstructionsPaneProps> = ({ width }) => {
  ...
  return (
    ...
    >
      ...
      {/* Tab Content */}
      <div className="flex-1 overflow-y-auto p-3 text-xs text-zinc-300 leading-relaxed space-y-3">
        {activeTab === 'theory' && (
          <div className="space-y-3">
            {/* Exercise Title & Instructions */}
            <div className="border border-zinc-800/80 bg-zinc-900/40 rounded-xl p-3">
              <h2 className="text-xs font-semibold text-zinc-100 mb-1 flex items-center gap-2">
                <span>{currentExercise.title}</span>
              </h2>
              {currentExercise.instructions ? (
                <div className="text-xs text-zinc-300 whitespace-pre-line bg-zinc-950/80 p-2.5 rounded-lg border border-zinc-800/60 font-mono text-[11px]">
                  {currentExercise.instructions}
                </div>
              ) : (
            ...
            {currentExercise.categoryReadme && (
              <div className="border border-zinc-800/80 bg-zinc-900/20 rounded-xl p-3.5 space-y-2">
                ...
                <div className="prose prose-invert prose-xs max-w-none text-zinc-300 [&_h1]:text-xs [&_h1]:font-bold [&_h2]:text-xs [&_h2]:font-semibold [&_h2]:mt-2.5 [&_h2]:mb-1 [&_p]:text-xs [&_p]:my-1.5 [&_ul]:my-1 [&_ul]:pl-4 [&_li]:my-0.5 [&_code]:bg-zinc-850 [&_code]:border [&_code]:border-zinc-800 [&_code]:px-1 [&_code]:py-0.2 [&_code]:rounded [&_code]:text-zinc-200 [&_a]:text-zinc-300 [&_a]:underline">
                  <ReactMarkdown
        ...
        {activeTab === 'hint' && (
          <div className="space-y-3">
            <div className="border border-zinc-800/80 bg-zinc-900/40 rounded-xl p-3.5 space-y-2.5">
              ...
              {hintRevealed ? (
                <div className="text-xs text-zinc-200 whitespace-pre-line leading-relaxed bg-zinc-950 p-3 rounded-lg border border-zinc-800 font-sans">
                ...
                >
                  <p className="text-xs text-zinc-300 font-medium">Click to reveal hint</p>
                  <p className="text-[10px] text-zinc-500 mt-1">Try to solve it without hints first</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="border border-zinc-800/80 bg-zinc-900/40 rounded-xl p-3.5 space-y-2.5">
              ...
              {solutionRevealed ? (
                <div className="space-y-2">
                  <pre className="p-3 bg-zinc-950 border border-zinc-800 rounded-lg text-[11px] font-mono text-zinc-200 overflow-x-auto select-text leading-snug">
                ...
                <div
                  ...
                  className="p-5 rounded-lg border border-dashed border-zinc-800 text-center cursor-pointer hover:bg-zinc-850/60 transition-colors"
                >
                  <p className="text-xs text-zinc-300 font-medium">Click to reveal official solution</p>
                  <p className="text-[10px] text-zinc-500 mt-1">Use this if you get stuck after checking hints</p>
                </div>
              )}
            </div>
          </div>
